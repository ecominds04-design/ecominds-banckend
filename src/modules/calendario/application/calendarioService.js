...
export const listarEventos = async (req) => {
  const { fechaInicio, fechaFin } = obtenerRango(req);
  ...
  return { fechaInicio, fechaFin, total: eventos.length, eventos };
};

export const crearEvento = async (req) => {
  ...
  if (!db.CalendarioEvento.TIPOS.includes(tipoEvento)) {
    throw new HttpError(400, `tipo debe ser uno de: ${db.CalendarioEvento.TIPOS.join(', ')}`);
  }
  ...
  if (auditoriaId) {
    ...
    if (!auditoria) {
      throw new HttpError(404, 'Auditoría no encontrada');
    }
    assertEmpresaInScope(auditoria.empresaId, req);
  }

  if (['servicio', 'producto'].includes(tipoEvento)) {
    throw new HttpError(400, 'Las asignaciones de servicios/productos se crean desde el módulo de asignaciones');
  }
  ...
  return evento.get({ plain: true });
};

export const actualizarEvento = async (id, req) => {
  const evento = await db.CalendarioEvento.findByPk(id);
  if (!evento) {
  ...
  if (evento.usuarioId && usuarioId && evento.usuarioId !== usuarioId) {
    throw new HttpError(403, 'No tienes permiso para editar este evento');
  }
...
export const eliminarEvento = async (id, req) => {
  const evento = await db.CalendarioEvento.findByPk(id);
  if (!evento) {
    throw new HttpError(404, 'Evento no encontrado');
  }
  ...
  const usuarioId = obtenerUsuarioId(req);
  if (evento.usuarioId && usuarioId && evento.usuarioId !== usuarioId) {
    throw new HttpError(403, 'No tienes permiso para eliminar este evento');
  }
...
export const crearAuditoriaDesdeCalendario = async (req) => {
  ...
  if (!auditorId) {
    throw new HttpError(401, 'No se pudo identificar al usuario autenticado');
  }
  ...
  return auditoria.get({ plain: true });
};